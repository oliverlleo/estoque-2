...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderFinancialTable(orcadoMap, negociadoMap, realizadoMap) {
        ...
        groupsArray.forEach(group => {
            ...
            // Agora realizadoMap é indexado por groupId (graças a alteração em calculateFinancials)
    ...
    function applyFilters() {
        ...
        const filteredItens = activeDataset.filter(item => {
            ...
            let grupoMatch = true;
            if (selectedGroupId) {
                // Se item.grupoId for indefinido, só combina se selectedGroupId for 'sem_grupo' (embora difícil acontecer se todos produtos tiverem grupo)
                // Usamos o ID salvo no item
                grupoMatch = (item.grupoId || 'sem_grupo') === selectedGroupId;
            }
