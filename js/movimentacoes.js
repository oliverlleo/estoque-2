...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const saidaFields = [
        ...
        document.getElementById('mov-estoque-locacao-wrapper')
    ];

    // --- Data Stores ---
    let locacoesCache = {}; // Cache for locations of a selected product
    let productsMap = {};
    let configData = {};
    let allMovements = [];
    ...
    async function updateProductInfo() {
        ...
        locacaoSelect.innerHTML = '<option value="">Primeiro, selecione um produto...</option>';
        ...
        try {
            ...
            if (locacoesSnapshot.empty) {
                ...
                locacoesSnapshot.forEach(doc => {
                    ...
                    locacoesCache[doc.id] = locacao;
                    const localNome = configData.locais[locacao.localId]?.nome || 'Desconhecido';
                    const displayText = `${localNome} - ${locacao.descricao}`;
                    locacaoSelect.innerHTML += `<option value="${doc.id}">${displayText}</option>`;
                });
                locacaoSelect.disabled = false;
            }
        } catch (error) {
            console.error("Erro ao buscar locações:", error);
            locacaoSelect.innerHTML = '<option value="">Erro ao carregar</option>';
        }
    ...
    document.getElementById('mov-locacao').addEventListener('change', () => {
        const locacaoId = document.getElementById('mov-locacao').value;
        const estoqueDisplay = document.getElementById('mov-estoque-locacao-display');
        if (locacaoId && locacoesCache[locacaoId]) {
            estoqueDisplay.textContent = locacoesCache[locacaoId].estoque || 0;
        } else {
            estoqueDisplay.textContent = '-';
        }
    });
                ...
                try {
                    ...
                    await runTransaction(db, async (transaction) => {
                        ...
                        if (tipoEntradaConfig && tipoEntradaConfig.movimenta_estoque === true) {
                            const newEstoqueLocacao = (locacaoDoc.data().estoque || 0) + quantidadeParaEstoque;
                            const newEstoqueTotal = (productDoc.data().estoque || 0) + quantidadeParaEstoque;
                ...
                try {
                    ...
                    await runTransaction(db, async (transaction) => {
                        ...
                        if (tipoSaidaConfig && tipoSaidaConfig.movimenta_estoque === true) {
                            ...
                            const newEstoqueTotal = (productDoc.data().estoque || 0) - quantidade;
                            transaction.update(locacaoRef, { estoque: newEstoqueLocacao });
                            transaction.update(productRef, { estoque: newEstoqueTotal });
                        }
