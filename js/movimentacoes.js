    ...
    productMovements.forEach(mov => {
        if (mov.tipo === 'entrada') {
            ...
            if (custoEntrada === undefined || custoEntrada === null) {
                ...
                // FORÇA conversão de string com vírgula para número (ex: "10,50" -> 10.50)
                const parseVal = (v) => {
                    if (typeof v === 'number') return v;
                    if (typeof v === 'string') return parseFloat(v.replace(',', '.')) || 0;
                    return 0;
                ...
                const qtdCompra = parseVal(mov.quantidade_compra);
                const valUnit = parseVal(mov.valor_unitario);
                const icms = parseVal(mov.icms);
                const ipi = parseVal(mov.ipi);
                const frete = parseVal(mov.frete);

                custoEntrada = (qtdCompra * valUnit) + icms + ipi + frete;
            }
            totalCost += custoEntrada;
