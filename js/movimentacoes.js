            ...
            try {
                ...
                await runTransaction(db, async (transaction) => {
                    ...
                    if (productDoc.exists()) {
                        ...
                        // Reverte a quantidade no estoque
                        if (tipoMovimento === 'entrada' || tipoMovimento === 'saida') {
                            if (movementData.locacao) {
                                ...
                                // Reverte a quantidade no estoque geral (sem locação)
                                let novoEstoqueGeral = productData.estoque || 0;
                                if (tipoMovimento === 'entrada') {
                                    novoEstoqueGeral -= quantidade;
                                } else { // 'saida'
                                    novoEstoqueGeral += quantidade;
                                }
                                transaction.update(productRef, { estoque: novoEstoqueGeral });
                            }
                        }
                    }
