...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    selectSobraOriginal.addEventListener('change', (e) => {
        ...
        // A linha de locação será removida do HTML
    });
    ...
    const renderTable = (data) => {
        ...
        data.forEach(product => {
            ...
            const locacoesDesc = locacoes.length > 0
                ? locacoes.map(l => `${configData.locais[l.localId]?.nome || ''} - ${l.descricao} (${l.estoque})`).join('<br>')
                : 'Nenhuma';
    ...
    // 4. Listen for real-time updates and fetch nested locations
    ...
    onSnapshot(q, (snapshot) => {
        const promises = snapshot.docs.map(async (pDoc) => {
            ...
            // Recalculate total stock based on locations
            product.data.estoque = product.locacoes.reduce((acc, loc) => acc + (loc.estoque || 0), 0);
            return product;
        });
    ...
    async function openLocacoesModal(productId) {
        ...
        locacoesModalProdutoInfo.innerHTML = `<strong>Produto:</strong> ${product.data.codigo} - ${product.data.descricao}`;
        ...
        unsubscribeLocacoes = onSnapshot(locacoesRef, (snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <button class="btn-delete btn-sm btn-delete-locacao" data-id="${doc.id}" ${locacao.estoque > 0 ? 'disabled' : ''}>Excluir</button>
                    </td>
                `;
    ...
    formAddLocacao.addEventListener('submit', async (e) => {
        e.preventDefault();
        const newLocacao = {
            localId: selectLocalId.value,
            descricao: document.getElementById('locacao-descricao').value,
            estoque: 0
        };
        if (!newLocacao.localId || !newLocacao.descricao) {
            alert("Por favor, preencha todos os campos da locação.");
            return;
        }
    ...
    tableLocacoesProdutoBody.addEventListener('click', async (e) => {
        if (e.target.classList.contains('btn-delete-locacao')) {
            ...
            if (confirm('Tem certeza que deseja excluir esta locação?')) {
                const locacaoRef = doc(db, `produtos/${currentProductIdForLocacao}/locacoes`, locacaoId);
                await deleteDoc(locacaoRef);
            }
        }
    });

    locacoesModalClose.onclick = () => {
    ...
    document.getElementById('btn-gerar-etiquetas').addEventListener('click', (e) => {
        ...
        const dadosParaEtiqueta = productsData
            ...
            .map(product => {
                // Since a product can have multiple locations, we don't pass a specific one.
                // This could be changed if a new business rule for labels is defined.
                return {
                    id: product.id,
                    data: product.data,
                    enderecamento: '' // Left blank for now
                };
            });
